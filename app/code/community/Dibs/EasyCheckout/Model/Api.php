...
class Dibs_EasyCheckout_Model_Api extends Mage_Core_Model_Abstract
{
     ...
     * @return array
     */
    public function createPayment(Mage_Sales_Model_Quote $quote, $fetchAddress = true)
    {
        $result = null;
        $paymentService = $this->getPaymentService();
        $createPaymentParams = $this->_getCreatePaymentParams($quote, $fetchAddress);
        $response = $paymentService->create($createPaymentParams);
        $result = $response->getResponseDataObject()->getData();
        return $result;
    }
     ...
     * @return array
     */
    protected function _getCreatePaymentParams(Mage_Sales_Model_Quote $quote, $fetchShippingAddress = true)
    {
        $helper = $this->_getDibsCheckoutHelper();
        $params = [
            'order' => [
           ...
           if (Mage::getModel('customer/session')->isLoggedIn()
               ...
               $params['checkout']['consumer'] =
                   ['email' => $this->getCustomer()->getEmail(),
                       ...
                       'privatePerson' => [
                           'firstName' => $helper->stringFilter($this->getCustomer()->getFirstname()),
                           'lastName' => $helper->stringFilter($this->getCustomer()->getLastname())]
                   ];
           }
       }
    ...
    {
        $helper = $this->_getDibsCheckoutHelper();
        $name = $helper->stringFilter($item->getName());
        $result = [
    ...
    protected function _getShippingLineItem($shippingInfo, $carrierReference, $carrierName)
    {
        ...
        $helper = $this->_getDibsCheckoutHelper();
        $name = $helper->stringFilter($carrierName);
        $result = [
    ...
    protected function getCustomerShippingAddress() 
    {
        ...
        if($customerSession->isLoggedIn()) {
           ...
           $shippingAddress = array(
               'addressLine1' => $helper->stringFilter($address->getStreet1()),
               'addressLine2'=> $helper->stringFilter($address->getStreet2()),
               'postalCode'=> $helper->stringFilter($address->getPostcode()),
               'city'=> $helper->stringFilter($address->getCity()),
