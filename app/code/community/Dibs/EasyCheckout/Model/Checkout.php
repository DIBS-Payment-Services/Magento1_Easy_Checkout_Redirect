...
class Dibs_EasyCheckout_Model_Checkout extends Mage_Core_Model_Abstract
{
     ...
     */
    public function createPayment(Mage_Sales_Model_Quote $quote, $fetchShippingAddress = true)
    {
        $result = null;
        /** @var Dibs_EasyCheckout_Model_Api $api */
        $api = Mage::getModel('dibs_easycheckout/api');
        $result = $api->createPayment($quote, $fetchShippingAddress);
    ...
    */
    ...
    public function placeOrder() {
        ...
        $order->setStatus($oderStatus)->setState('processing');
        // Set Order Dibs Payment Id
        $order->setDibsEasyPaymentId($quote->getDibsEasyPaymentId()); //$quote->getDibsEasyPaymentId());
    ...
    public function cancelOrder() {
        ...
        $order = Mage::getModel('sales/order')
            ->loadByIncrementId($lastOrderId);
        $order->cancel();
        $history = $order->addStatusHistoryComment('Cancelled by customer', false);
