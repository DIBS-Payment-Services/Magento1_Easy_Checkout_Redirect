...
class Dibs_EasyCheckout_CheckoutController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
        ...
        try {
            ...
            if (empty($paymentId)) {
                $paymentId = $dibsCheckout->createPayment($helper->getQuote());
            }
    ...
    public function returnAction()
    {
        ...
        if (empty($paymentId) || $quote->getDibsEasyPaymentId() != $paymentId) {
            return $this->_redirect('checkout/cart');
        }
       /** @var Dibs_EasyCheckout_Model_Api $api */
        $api = Mage::getModel('dibs_easycheckout/api');
        try {
            $payment = $api->findPayment($paymentId);
            if(!empty($payment->getPaymentDetails()->getPaymentType())) {
                if($quote->getDibsEasyPaymentId() == $paymentId) {
                    $dibsCheckout->placeOrder();
                    return $this->_redirect('checkout/onepage/success', array('_secure'=>true));
                } else {
                    $message = $helper->__('There is error. Please contact store administrator for details');
                    $helper->getCheckout()->addError($message);
            ...
            } else { //cancelling the payment
                $message = $helper->__('The payment has been cancelled');
                $helper->getCheckout()->addSuccess($message);
                $dibsCheckout->cancelOrder();
                return $this->_redirect('checkout/cart');
            }
        }catch (Exception $e) {
            $message = $helper->__('There is error. Please contact store administrator for details');
